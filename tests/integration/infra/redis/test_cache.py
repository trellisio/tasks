...
class TestRedisCache:
    ...
    async def set_up(self) -> None:  # noqa: PT004
    ...
    async def test_can_set_value(self) -> None:
        res = await self.cache.set(key="service", value="service_name")
        assert res is True

    async def test_can_set_values(self) -> None:
        res = await self.cache.multi_set(
            {"service": "service_name", "name": "developer", "company": "trellis"},
        )
        assert res is True

    async def test_can_cache_value(self) -> None:
        await self.cache.set(key="service", value="service_name")
        res = await self.cache.get("service")
        assert res == "service_name"

    async def test_can_cache_values(self) -> None:
    ...
    async def test_can_delete_value(self) -> None:
        await self.cache.set(key="service", value="service_name")
        await self.cache.delete("service")
        res = await self.cache.get("service")
        assert res is None

    async def test_can_delete_values(self) -> None:
        await self.cache.multi_set(
            {"service": "service_name", "name": "developer", "company": "trellis"},
        )
