...
class AddProcessTimeHeader(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next: CallNext) -> Response:  # noqa: PLR0917, PLR6301
...
class AddRequestLogger(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next: CallNext) -> Response:  # noqa: PLR0917
        try:
            response = await call_next(request)
        ...
        self._log_by_color(
            log_type="info",
            message=f"[{request.method}] {request.url} {response.status_code} - {response.headers["X-Process-Time"]}",
    ...
    @staticmethod
    def _get_color(status_code: int) -> str:
        if status_code < 300:
            return "green"
        if status_code < 500:
            return "yellow"
